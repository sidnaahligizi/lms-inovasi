...
export async function POST(req) {
  try {
    ...
    const orgName = process.env.TURSO_ORG_NAME ? process.env.TURSO_ORG_NAME.trim().toLowerCase() : ''; 
    const platformToken = process.env.TURSO_PLATFORM_TOKEN ? process.env.TURSO_PLATFORM_TOKEN.trim() : '';

    if (!orgName || !platformToken) {
       throw new Error("Konfigurasi TURSO_ORG_NAME atau TURSO_PLATFORM_TOKEN belum diatur di Vercel.");
    }
    ...
    // 2. Generate Token Autentikasi untuk DB Baru
    const createTokenRes = await fetch(`https://api.turso.tech/v1/organizations/${orgName}/databases/${dbName}/auth/tokens`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${platformToken}` }
    });

    if (!createTokenRes.ok) throw new Error("Gagal generate Token Auth Database.");

    const tokenData = await createTokenRes.json();
    // Menggunakan HTTPS agar lebih stabil terhubung dari Vercel
    const tenantDbUrl = `https://${dbName}-${orgName}.turso.io`;
    ...
    // 6. Buat Akun Admin Pertama di DB Sekolah
