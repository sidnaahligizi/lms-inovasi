...
export async function POST(req) {
  try {
    ...
    const checkQuota = async (newSiswaCount, targetSekolah) => {
        ...
        if (current + newSiswaCount > limit) { throw new Error(`Gagal Menambahkan: Kapasitas penuh (Maksimal ${limit} Siswa). Silakan hubungi pengembang untuk Upgrade Paket Institusi Anda.`); }
    };
    ...
    if (action === 'adminManageUser') {
      ...
      if (mode === 'save') {
        ...
        if (cek.rows.length > 0) { await turso.execute({ sql: `UPDATE Users SET Nama=?, Username=?, Password=?, Role=?, Kelas=?, TglLahir=?, NISN=? WHERE ID=? OR id=?`, args: [d.nama, d.username, d.password, d.role, d.kelas, d.tglLahir, d.nisn, id, id] }); } 
        else { await turso.execute({ sql: `INSERT INTO Users (ID, Nama, Username, Password, Role, Sekolah, Kelas, TglLahir, NISN) VALUES (?,?,?,?,?,?,?,?,?)`, args: [id, d.nama, d.username, d.password, d.role, sekolah, d.kelas, d.tglLahir, d.nisn] }); }
    ...
    // --- KUSTOMISASI LMS (Mendaur ulang kolom DbUrl dan DbToken) ---
    if (action === 'updateTenantSettings') {
        const [namaLmsBaru, logoBase64] = args;
        await turso.execute({ sql: "UPDATE Tenants SET DbUrl = ?, DbToken = ? WHERE NamaSekolah = ? OR namasekolah = ?", args: [logoBase64, namaLmsBaru, sekolah, sekolah] });
        return NextResponse.json({ status: 'success', msg: 'Pengaturan berhasil disimpan! Silakan Login Ulang agar logo berubah.' });
    }
    ...
    if (action === 'submitAbsen') {
      ...
      await turso.execute({ sql: "INSERT INTO Attendance (AbsenID, SiswaID, Tanggal, Status, Sekolah) VALUES (?, ?, ?, 'Hadir', ?)", args: ['ABS' + Date.now(), userId, dateNow, sekolah] });
      return NextResponse.json({ status: 'success', msg: 'Berhasil melakukan absensi hari ini!' });
    }

    // --- FITUR CETAK RAPORT ---
    if (action === 'getRaportData') {
        const [r_role, r_sekolah, r_kelas] = args;
    ...
    // --- DATA UJIAN & DASHBOARD ---
    if (action === 'getDashboardData') {
      let output = { logo: 'https://lh3.googleusercontent.com/d/1aWHmp6kNKwTYkwEMqVg34_ofiWRkymFe' };
      
      const tenantInfo = await turso.execute({ sql: "SELECT Paket, DbUrl, DbToken FROM Tenants WHERE NamaSekolah = ?", args: [sekolah] });
      if (tenantInfo.rows.length > 0) {
          output.paket = tenantInfo.rows[0].Paket || tenantInfo.rows[0].paket;
          // Menyuntikkan Setingan Kustom
          output.customLogo = tenantInfo.rows[0].DbUrl || tenantInfo.rows[0].dburl;
          output.customName = tenantInfo.rows[0].DbToken || tenantInfo.rows[0].dbtoken;
      }
